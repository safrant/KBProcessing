...
int main (int argc, char * argv[])
{
	...
	posSub = subsets.begin();
        ...
        outFile << "Source\tSubset\tNCI Concept Code\tFDA PT\tFDA PT Source Code\tFDA Definition";
	outFile	<< "\tFDA Synonym (Source Code)\tUse_For (Code)\tParent Concept FDA PT\tParent Concept NCI Code";
	outFile << endl;
    								...
    								if( posQual->getName() == "Syn_Source" && posQual->getValue() == source && posProp->second.getValue() != sourcePT ) {
    									...
    									synExists = true;
             							...
             							if( posQual->getName() == "Syn_Source_Code" ) {
                                            ...
                                            outFile << source << "\t" << subset << "\t" << posCon->second.getCode() << "\t";
                                            outFile << sourcePT << "\t" << sourceCode << "\t" << sourceDef << "\t" << sourceSyns;
                                            outFile << "\t" << useForBlurb << "\t" << parent << "\t" << parentCode << endl;
                                        }
                                }
                        }
                }
